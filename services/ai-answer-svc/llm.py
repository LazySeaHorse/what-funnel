...
async def get_ai_config(db: ScopedDB) -> Tuple[str, str, str, str]:
    ...
    try:
        ...
        raise ValueError("Failed to decrypt AI provider configuration.")
...
async def embed(api_key: str, base_url: str, model: str, text: str) -> List[float]:
    ...
    async with httpx.AsyncClient(timeout=None) as client:
        try:
            ...
            raise RuntimeError(f"Failed to generate embeddings from AI provider: {e}")
        ...
        try:
            ...
            raise RuntimeError("Invalid response structure from AI provider embeddings endpoint.")

        if len(embedding) != 1536:
            raise ValueError(f"Unsupported embedding dimension: expected 1536, got {len(embedding)}.")
...
async def complete(api_key: str, base_url: str, model: str, messages: List[dict], response_schema: Any) -> dict:
    ...
    # We alter the messages to append schema requirements to the last message or format appropriately
    system_message = {"role": "system", "content": f"You are a helpful assistant that always outputs JSON matching this schema:\n{schema_json}"}
    ...
    async with httpx.AsyncClient(timeout=None) as client:
        ...
        try:
            ...
            raise ValueError(f"AI provider response failed schema validation: {e}")
