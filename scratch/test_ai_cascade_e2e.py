...
sys.path.insert(0, '/home/ubuntu/what-funnel/services/ai-answer-svc')
...
KB_COMPILER_URL = "http://localhost:8085"
...
async def run_e2e_test():
    ...
    await conn.execute(
        """
        INSERT INTO patterns (id, account_id, title, trigger_phrases, answer_markdown, embedding)
        VALUES 
        ...
        uuid.uuid4(), account_id, ["corporate headquarters address", "where is your office located"], str(hq_emb)
    )
    ...
    async with httpx.AsyncClient() as client:
        ...
        added_concepts = compile_data.get("added_concepts", [])
        suggestion_ids = compile_data.get("suggestion_ids", [])
    ...
    for idx, tc in enumerate(test_cases, 1):
        ...
        await conn.execute(
            """
            INSERT INTO conversations (id, account_id, channel_type, channel_identifier, ai_mode_active)
            VALUES ($1, $2, 'whatsapp', '+15551234567', true)
            """,
            convo_id, account_id
        )
        ...
        if tc["has_prior_human"]:
            ...
            await conn.execute(
                ...
                VALUES ($1, $2, $3, 'outbound', 'human', 'text', $4)
                """,
                uuid.uuid4(), account_id, convo_id,
        ...
        await conn.execute(
            """
            INSERT INTO messages (id, account_id, conversation_id, direction, sender_type, content_type, content)
            VALUES ($1, $2, $3, 'inbound', 'customer', 'text', $4)
            """,
        ...
        # Wait for ai-answer-svc to process cascade
        await asyncio.sleep(4.0)

        # Fetch AI answer event from DB
        ...
        reply_msg = None
        if event and event["reply_message_id"]:
