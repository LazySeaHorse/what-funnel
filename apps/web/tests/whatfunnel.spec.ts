...
function injectInboundMessage(opts: {
  ...
  const payload = {
    ...
    message: {
      ...
      direction: "inbound",
      sender: {
        external_id: opts.externalIdentity,
...
async function signupViaPage(page: Page, email: string, businessName: string) {
  ...
  await expect(
    page.getByRole("radio", { name: /Full workspace/i }),
  ).toBeChecked();
...
async function continueToStep(page: Page, step: number) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      // The first server-rendered click can race Svelte hydration.
    }
  }
...
/** Complete onboarding without requiring a live AI provider credential. */
async function completeOnboarding(page: Page) {
  ...
  await page.getByText("Manual only", { exact: true }).click();
...
async function loginViaPage(page: Page, email: string) {
  ...
  await page.fill("#identifier-input", email);
...
async function createChannel(page: Page): Promise<string> {
  ...
  const connectionResponsePromise = page.waitForResponse(
    (resp) =>
      resp.url().includes("/channel-connections") &&
...
async function createSimulatorChannel(page: Page): Promise<string> {
  const cookies = await page.context().cookies();
  const csrfToken = cookies.find(
    (cookie) => cookie.name === "csrf_token",
  )?.value;
  const response = await page.request.post(
    "http://localhost:18080/simulate/channels",
    {
...
test.describe("1. Auth & Session", () => {
  test("1.1 Signup creates account and auto-advances to onboarding", async ({
    page,
  }) => {
    await page.goto("/signup");
    await page.waitForLoadState("networkidle");
  ...
  test("1.2 Login with valid credentials succeeds and lands in inbox", async ({
    page,
  }) => {
  ...
  test("1.3 Login with wrong credentials stays on /login", async ({ page }) => {
    await page.goto("/login");
    await expect(page.locator("form")).toBeVisible({ timeout: 10000 });
    await page.fill("#identifier-input", "nobody-fake@does-not-exist.local");
...
test.describe("2. Onboarding Flow", () => {
  test("2.1 Full onboarding flow completes and arrives in inbox", async ({
    page,
  }) => {
  ...
  test("2.3 Step 2 presents available messaging channels", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: "Connect messaging channels",
        exact: true,
      }),
    ).toBeVisible();
  ...
  test("2.4 Setup complete screen sends the user to the inbox", async ({
    page,
  }) => {
  ...
  test("2.5 Step 5 offers the current AI assistant modes", async ({ page }) => {
    await signupViaPage(page, uniqueEmail("ob6"), "Reply Mode Test");
    await page.waitForURL("**/onboarding/1", { timeout: 20000 });
    for (const step of [2, 3, 4, 5]) {
      await continueToStep(page, step);
    }
    await expect(
      page.getByRole("heading", {
    ...
    await expect(
      page.getByRole("button", { name: /Auto answer when confident/ }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Suggest replies only/ }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Manual only/ }),
    ).toBeVisible();
  });
});
...
test.describe("3. Channel Management", () => {
  ...
  test("3.1 In-app Settings opens the Channels section", async ({ page }) => {
    ...
    await page.getByRole("tab", { name: "Channels", exact: true }).click();
    await expect(
      page.getByRole("heading", { name: "Messaging accounts", exact: true }),
    ).toBeVisible({ timeout: 15000 });
  });

  test("3.2 Connect WhatsApp dialog requests an account label", async ({
    page,
  }) => {
    await loginViaPage(page, suiteEmail);
    await page.goto("/inbox?tab=settings");
  ...
  test("3.3 Creating a channel updates the settings list", async ({ page }) => {
    ...
    const unlinkResponse = page.waitForResponse(
      (response) =>
        response.url().includes(`/channel-connections/${channelID}`) &&
        response.request().method() === "DELETE",
    );
    await page.getByRole("button", { name: "Unlink" }).click();
    expect((await unlinkResponse).status()).toBe(204);
    await expect(
      page.getByText("No WhatsApp accounts connected yet.", { exact: true }),
    ).toBeVisible({ timeout: 5000 });
  });

  test("3.4 Disconnecting a channel returns the view to its empty state", async ({
    page,
  }) => {
    await loginViaPage(page, suiteEmail);
    const channelID = await createChannel(page);
...
test.describe("4. Inbound Message Flow (Demo Mode)", () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    await completeOnboarding(page);
    channelId = await createSimulatorChannel(page);
    await page.close();
  });
  ...
  test("4.1 Inbound message appears in inbox conversation list", async ({
    page,
  }) => {
    ...
    injectInboundMessage({
      channelId,
      externalThreadID: `t-${Date.now()}`,
      externalIdentity: `wa-${Date.now()}@s.whatsapp.net`,
      displayName,
      text: "Do you offer house calls?",
    });
    await expect(
      page.locator(`.convo-item:has-text("${displayName}")`),
    ).toBeVisible({ timeout: 20000 });
  });

  test("4.2 Opening conversation shows inbound message text", async ({
    page,
  }) => {
    await goToInboxAllTab(page);
    ...
    await item.click();
    await expect(page.locator(`.msg-text:has-text("${msgText}")`)).toBeVisible({
      timeout: 10000,
    });
  });

  test("4.3 Multiple messages from same contact land in same conversation", async ({
    page,
  }) => {
    await goToInboxAllTab(page);
    ...
    injectInboundMessage({
      ...
      externalIdentity: extId,
      displayName,
      text: "Are you open weekends?",
    });
    const item = page.locator(`.convo-item:has-text("${displayName}")`);
    await expect(item.first()).toBeVisible({ timeout: 20000 });
    await item.first().click();
    await expect(page.locator('.msg-text:has-text("Hello!")')).toBeVisible({
      timeout: 10000,
    });
    await expect(
      page.locator('.msg-text:has-text("Are you open weekends?")'),
    ).toBeVisible({ timeout: 8000 });
  });

  test("4.4 Inbound message row does not have the outbound class", async ({
    page,
  }) => {
    await goToInboxAllTab(page);
    ...
    await item.click();
    await expect(
      page.locator(".message-row:not(.outbound)").first(),
...
test.describe("5. Outbound Messaging", () => {
  ...
  async function loginAndOpenConvo(
    page: Page,
    displayName: string,
    inboundText: string,
  ) {
  ...
  test("5.1 Can type and send a reply", async ({ page }) => {
    ...
    await expect(page.locator(`.msg-text:has-text("${reply}")`)).toBeVisible({
      timeout: 10000,
    });
  });

  test("5.2 Sent message has outbound class in the thread", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator(
        '.message-row.outbound .msg-text:has-text("Yes, we deliver")',
      ),
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("5.4 Enter key sends the message", async ({ page }) => {
    ...
    await expect(page.locator(`.msg-text:has-text("${msg}")`)).toBeVisible({
      timeout: 10000,
    });
  });
...
test.describe("6. Lead Tracking (Full Workspace)", () => {
  ...
  async function ensureLeadStarted(page: Page) {
    await expect(
      page.getByRole("button", { name: "Change lead stage" }),
    ).toBeVisible({ timeout: 8000 });
  }

  test("6.1 Lead panel visible when conversation open in full_workspace", async ({
    page,
  }) => {
    await openFreshLeadConvo(page);
    await expect(
      page.locator(".lead-panel").getByText("Lead stage", { exact: true }),
    ).toBeVisible();
  });

  test("6.2 Lead stage control is available for a new conversation", async ({
    page,
  }) => {
    await openFreshLeadConvo(page);
  ...
  test("6.3 Can add a note to a lead", async ({ page }) => {
    ...
    await expect(page.locator(".lead-panel").getByText(note)).toBeVisible({
      timeout: 8000,
    });
  });
  ...
  test("6.5 Can change lead pipeline stage", async ({ page }) => {
    await openFreshLeadConvo(page);
    await ensureLeadStarted(page);
    await page.getByRole("button", { name: "Change lead stage" }).click();
    const options = page.locator('button[aria-label^="Set lead stage to "]');
    ...
    await expect(
      ...
      (nextState || "").replace("Set lead stage to ", "").toLowerCase(),
    );
  });

  test("6.6 History tab shows entries after a state change", async ({
    page,
  }) => {
    await openFreshLeadConvo(page);
    await ensureLeadStarted(page);
    await page.getByRole("button", { name: "Change lead stage" }).click();
    await page
      .locator('button[aria-label^="Set lead stage to "]')
      .nth(1)
      .click();
    ...
    await expect(
      page
        .locator(".lead-panel")
        .getByText(/Lead stage changed to|No stage history recorded/)
        .first(),
    ).toBeVisible({ timeout: 8000 });
  });
});
...
test.describe("7. Inbox Filters & Navigation", () => {
  ...
  test("7.2 Admin sees All / Mine / Unassigned filter tabs", async ({
    page,
  }) => {
    await loginViaPage(page, suiteEmail);
    await expect(page.locator('button.tab-btn:has-text("All")')).toBeVisible({
      timeout: 10000,
    });
    await expect(page.locator('button.tab-btn:has-text("Mine")')).toBeVisible();
    await expect(
      page.locator('button.tab-btn:has-text("Unassigned")'),
    ).toBeVisible();
  });

  test("7.3 Clicking a filter tab changes the active state", async ({
    page,
  }) => {
    await loginViaPage(page, suiteEmail);
    await expect(page.locator("button.tab-btn").first()).toBeVisible({
      timeout: 10000,
    });
    await page.click('button.tab-btn:has-text("Unassigned")');
    await expect(
      page.locator('button.tab-btn.active:has-text("Unassigned")'),
    ).toBeVisible({ timeout: 3000 });
  });

  test("7.4 Inbound conversations appear under All filter", async ({
    page,
  }) => {
    await loginViaPage(page, suiteEmail);
    ...
    await expect(page.locator(`.convo-item:has-text("${dn}")`)).toBeVisible({
      timeout: 20000,
    });
  });
...
test.describe("8. Settings Pages", () => {
  ...
  test("8.1 Admin can navigate every current settings section", async ({
    page,
  }) => {
...
test.describe("9. WebSocket Realtime Push", () => {
  ...
  test("9.1 Inbox loads without JS errors after WS connect", async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on("console", (m) => {
    ...
    const unexpected = errors.filter(
      (e) => !e.includes("unauthenticated") && !e.includes("favicon"),
    );
    expect(unexpected).toHaveLength(0);
  });

  test("9.2 New inbound message appears in list via WS without page refresh", async ({
    page,
  }) => {
    await loginViaPage(page, suiteEmail);
    const allBtn = page.locator('button.tab-btn:has-text("All")');
    ...
    injectInboundMessage({
      ...
      externalIdentity: `wa-${Date.now()}@s.whatsapp.net`,
      displayName: dn,
      text: "Real-time push test",
    });
    await expect(page.locator(`.convo-item:has-text("${dn}")`)).toBeVisible({
      timeout: 20000,
    });
    expect(await page.locator(".convo-item").count()).toBeGreaterThanOrEqual(
      before,
    );
  });

  test("9.3 Sending a reply — outbound message appears in thread", async ({
    page,
  }) => {
    await loginViaPage(page, suiteEmail);
    ...
    await page.click("button.send-btn");
    await expect(page.locator(`.msg-text:has-text("${outMsg}")`)).toBeVisible({
      timeout: 10000,
    });
  });
});
...
test.describe("10. RBAC — Admin vs Member", () => {
  test("10.1 Admin sees the in-app Settings control in the inbox sidebar", async ({
    page,
  }) => {
  ...
  test("10.2 Admin can open the Channels section without leaving the inbox shell", async ({
    page,
  }) => {
