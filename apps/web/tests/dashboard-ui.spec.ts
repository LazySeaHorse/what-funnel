...
test("dashboard UI renders correctly with What Funnel branding and Poppins font", async ({
  page,
}) => {
  ...
  await page.waitForURL(
    (url) =>
      url.pathname.includes("/onboarding") || url.pathname.includes("/inbox"),
    { timeout: 20000 },
  );
...
test("leads tab UI renders real database leads with table and detail drawer", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  ...
  const presetBtn = page.locator(
    'button:has-text("Hi! Do you have any weekend slots available?")',
  );
  await presetBtn.click();
  await expect(
    page.locator("text=Inbound message sent to What Funnel"),
  ...
  await page.fill(
    ...
    "Customer prefers Saturday afternoon.",
  );
  await page.getByRole("button", { name: "Save", exact: true }).click();
  await expect(
    page.locator("text=Customer prefers Saturday afternoon."),
  ...
  await expect(
    page
      .locator("text=Filter by Stage")
      .or(page.locator('label:has-text("Channel")')),
  ).toBeVisible();
