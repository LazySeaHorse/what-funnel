...
test.describe("in-app settings safety net", () => {
  async function openMockedSettings(
    page: Parameters<typeof mockWorkspaceApi>[0],
  ) {
  ...
  test("every section is reachable and exposes the matching accessible panel", async ({
    page,
  }) => {
  ...
  test("a manager can save a BYOK provider configuration without the key being displayed again", async ({
    page,
  }) => {
  ...
  test("a failed save preserves edits and gives the user a recoverable error", async ({
    page,
  }) => {
    await openMockedSettings(page);
    await page.route("**/api-gateway/workspace/account/settings", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({ error: "Settings service is unavailable" }),
      }),
    );
    ...
    await expect(
      page.getByText("Settings service is unavailable", { exact: true }),
    ).toBeVisible();
    await expect(page.getByLabel("Default time zone")).toHaveValue("UTC");
  });

  test("add user modal can be dismissed by keyboard without changing workspace state", async ({
    page,
  }) => {
    await openMockedSettings(page);
    await page
      .getByRole("tab", { name: "Users & permissions", exact: true })
      .click();
  ...
  test("user dialogs complete add, credential, reset, and delete workflows", async ({
    page,
  }) => {
    const api = await mockWorkspaceApi(page);
    await page.goto("/inbox?tab=settings");
    ...
    await expect(
      page.locator(".font-medium.text-slate-800", { hasText: "newagent" }),
    ).not.toBeVisible();
    ...
    expect(api.requests).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          path: "/workspace/users/user-2/password",
          method: "PUT",
        }),
        expect.objectContaining({
          path: "/workspace/users/user-2",
          method: "DELETE",
        }),
      ]),
    );
  ...
  test("channel connection dialog can be cancelled with Escape", async ({
    page,
  }) => {
    await openMockedSettings(page);
    ...
    await expect(
      page.getByRole("dialog", { name: "Connect WhatsApp" }),
    ).toBeVisible();
    await page.keyboard.press("Escape");
    await expect(
      page.getByRole("dialog", { name: "Connect WhatsApp" }),
  ...
  test("deleting a provider channel removes it and its associated chats", async ({
    ...
    await expect(
      page.getByText("Sales WhatsApp", { exact: true }),
    ).toBeVisible();

    page.once("dialog", (dialog) => {
      expect(dialog.message()).toContain(
        "chats and messages will be permanently deleted",
      );
      void dialog.accept();
    });
    await page.getByRole("button", { name: "Unlink", exact: true }).click();
    await expect(
      page.getByText("Sales WhatsApp was unlinked.", { exact: true }),
    ).toBeVisible();
    await expect(
  ...
  test("a failed provider connection keeps the selected channel ready to retry", async ({
    page,
  }) => {
    await openMockedSettings(page);
    await page.getByRole("tab", { name: "Channels", exact: true }).click();
    await page.route("**/api-gateway/channel-connections", (route) => {
      if (route.request().method() === "POST") {
        return route.fulfill({
          status: 503,
          contentType: "application/json",
          body: JSON.stringify({ error: "Bridge service is unavailable" }),
        });
      }
    ...
    await expect(
      page.getByText("Bridge service is unavailable", { exact: true }),
    ).toBeVisible();
    await expect(dialog).toBeVisible();
    await expect(dialog.getByLabel("Account label")).toHaveValue(
      "Support WhatsApp",
    );
  });
  ...
  test("workspace type changes hide lead-pipeline controls when lead tracking is unavailable", async ({
    ...
    await openMockedSettings(page);
    await expect(
      page.getByRole("tab", { name: /Lead pipeline/ }),
    ).toBeVisible();
    await page.getByRole("radio", { name: /Chatbot only/ }).check();
    await expect(
      page.getByText("Workspace type updated.", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("tab", { name: /Lead pipeline/ }),
    ).not.toBeVisible();
    await expect(
      page.getByRole("button", { name: "Leads", exact: true }),
    ).not.toBeVisible();
    await expect(
  ...
  test("pipeline editor generates unique keys for duplicate stage labels", async ({
    page,
  }) => {
    const api = await mockWorkspaceApi(page);
    await page.goto("/inbox?tab=settings");
    ...
    await expect(
      page.getByText("Pipeline saved.", { exact: true }),
    ).toBeVisible();
    const save = api.requests.find(
      (request) =>
        request.path === "/workspace/pipelines/pipeline-1" &&
        request.method === "PUT",
    );
  ...
  test("workspace deletion cannot be submitted until the exact workspace name is entered", async ({
    page,
  }) => {
    await openMockedSettings(page);
  ...
  test("the settings view remains operable without horizontal overflow on mobile", async ({
    page,
  }) => {
  ...
  test("visible settings buttons always have an accessible name", async ({
    page,
  }) => {
    await mockWorkspaceApi(page);
    await page.goto("/inbox?tab=settings");
