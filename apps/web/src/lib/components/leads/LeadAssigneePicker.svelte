	...
	{#if open}
		<div transition:fly={{ y: -4, duration: 120, easing: cubicOut }} class="absolute top-full left-0 mt-1 w-52 bg-white rounded-xl border border-slate-200 shadow-md py-1 z-50 text-xs">
			...
			{#each users as user (user.id)}
				...
				<button type="button" onclick={() => void onToggle(user.id)} title={displayName} class="w-full flex items-center justify-between px-3 py-2 text-left hover:bg-slate-50 font-medium cursor-pointer {assigned ? 'text-blue-600 bg-blue-50/50' : 'text-slate-700'}">
