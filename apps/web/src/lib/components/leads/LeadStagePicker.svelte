...
<div class="space-y-1.5">
	...
	<Popover bind:open width="w-full" class="w-full">
		{#snippet trigger()}
			<button
				type="button"
				onclick={() => (open = !open)}
				aria-label="Change lead stage"
				class="w-full flex items-center justify-between p-2.5 bg-slate-50/80 rounded-xl border border-slate-200/80 cursor-pointer hover:bg-slate-100/60 transition text-left"
			>
		...
		{#each states as state (state.key)}
			<button
				type="button"
				onclick={() => { open = false; void onchange(state.key); }}
