...
export class SimulatorController {
  ...
  testContacts = $state<TestContact[]>(
    DEFAULT_CONTACTS.map((contact) => ({ ...contact })),
  );
  ...
  dispose() {
    ...
    if (this.sentHandler)
      window.removeEventListener("dev-message-sent", this.sentHandler);
  ...
  async sendMessage(text: string) {
    ...
    const targetPlatform =
      this.selectedPlatform || contact.platform || "whatsapp";
    ...
    try {
      const nativePayload = buildNativePayload(
        targetPlatform,
        contact,
        textToSend,
      );
  ...
  async refreshCurrentConversation(showLoading = true) {
    ...
    try {
      const snapshot = await this.loadConversationSnapshot(
        contactSnapshot,
        platform,
      );
      if (
        requestVersion !== this.conversationLoadVersion ||
        this.selectedContactID !== snapshot.contactID ||
        this.selectedPlatform !== snapshot.platform
      )
        return;
      ...
      if (showLoading && requestVersion === this.conversationLoadVersion)
        this.isConvoLoading = false;
    }
  }
  ...
  private async ensureChannelForPlatform(
  ...
  private async loadConversationSnapshot(
    contact: TestContact,
    platform: SimulatorPlatform,
  ): Promise<ConversationSnapshot> {
    const conversations = await apiRequest("/conversations?filter=all");
    const match = findConversation(
    ...
    return {
      ...
      messages: Array.isArray(messageResponse?.messages)
        ? [...messageResponse.messages].reverse()
        : [],
      telemetry: draft
        ? {
  ...
  private persistContacts() {
    try {
      localStorage.setItem(
        SIMULATOR_STORAGE_KEY,
        JSON.stringify(this.testContacts),
      );
    } catch {
  ...
  return conversations.find(
    (conversation) =>
      (conversation.contact_name &&
        (conversation.contact_name.startsWith(contact.name) ||
          conversation.contact_name.includes(contact.name))) ||
      (conversation.contact?.display_name &&
        (conversation.contact.display_name.startsWith(contact.name) ||
          conversation.contact.display_name.includes(contact.name))) ||
      conversation.contact?.external_identity === contact.externalID ||
      conversation.external_identity === contact.externalID ||
      (conversation.display_name &&
        (conversation.display_name.startsWith(contact.name) ||
          conversation.display_name.includes(contact.name))) ||
      (conversation.contact_display_name &&
        (conversation.contact_display_name.startsWith(contact.name) ||
          conversation.contact_display_name.includes(contact.name))),
  );
}
