<script lang="ts">
	import { CheckIcon } from '@fvilers/heroicons-svelte/20/solid';
...
<div class="hidden lg:flex w-full lg:w-80 xl:w-96 bg-slate-50 border-b lg:border-b-0 lg:border-r border-slate-200/80 flex-col justify-between shrink-0 p-6 lg:p-10 relative overflow-hidden min-h-[480px] lg:min-h-0">
	<div class="relative z-10">
		...
		{#if stepNum === 8}
			<h1 class="text-2xl sm:text-3xl font-medium text-slate-900 leading-snug tracking-tight mb-3">Workspace is<br /><span class="text-blue-600">ready</span></h1>
			<p class="text-sm text-slate-500 leading-relaxed mb-6 font-normal">Setup is complete and channels are connected.</p>
		{:else}
			<h1 class="text-2xl sm:text-3xl font-medium text-slate-900 leading-snug tracking-tight mb-3">Set up<br /><span class="text-blue-600">your workspace</span></h1>
			<p class="text-sm text-slate-500 leading-relaxed mb-6 font-normal">Configure your workspace settings in the following steps.</p>
		{/if}
	...
	<div class="absolute inset-x-0 bottom-0 w-full overflow-hidden pointer-events-none flex flex-col justify-end">
		...
		<img src={stepNum === 8 ? '/images/onboarding-happy.webp' : '/images/onboarding-sidebar.webp'} alt={stepNum === 8 ? 'Setup complete illustration' : 'Workspace illustration'} class="w-full h-auto max-h-80 object-cover object-bottom [mask-image:linear-gradient(to_bottom,transparent_0%,black_35%)]" />
	</div>
</div>
		...
		{#each stepItems as item}
			...
			<button type="button" class="flex items-center gap-3 bg-transparent border-0 p-0 text-left cursor-pointer transition shrink-0 {isActive ? 'opacity-100' : isDone ? 'opacity-85' : 'opacity-50'}" onclick={() => { if (item.num <= stepNum) onStep(item.num); }}>
				<div class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium shrink-0 transition-all {isActive ? 'bg-blue-600 text-white shadow-xs' : isDone ? 'bg-blue-600 text-white' : 'border border-slate-300 text-slate-400 bg-white'}">
					{#if isDone}<CheckIcon class="w-3.5 h-3.5" />{:else}<span>{item.num}</span>{/if}
				</div>
