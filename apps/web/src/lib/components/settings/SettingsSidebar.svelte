...
<div class="col-span-12 md:col-span-3 lg:col-span-2">
  ...
  >
    {#each items as item}
      {#if item.key !== "users_permissions" || canManageTeam}
        ...
        <button
          ...
          class="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium transition-all text-left {activeSection ===
          item.key
            ? 'bg-blue-50/80 text-blue-600 shadow-2xs'
            : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}"
        >
          <IconComponent
            class="w-4 h-4 {activeSection === item.key
              ? 'text-blue-600'
              : 'text-slate-400'}"
          />
    ...
    {#if productMode === "full_workspace"}
      ...
      >
        <FunnelIcon
          class="w-4 h-4 {activeSection === 'pipeline'
            ? 'text-blue-600'
            : 'text-slate-400'}"
        />
