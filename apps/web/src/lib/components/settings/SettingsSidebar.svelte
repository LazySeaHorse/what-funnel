<script lang="ts">
	type SettingsSection = 'general' | 'business_profile' | 'users_permissions' | 'channels' | 'pipeline';
	let { activeSection = $bindable<SettingsSection>(), productMode }:
		{ activeSection: SettingsSection; productMode: string } = $props();
