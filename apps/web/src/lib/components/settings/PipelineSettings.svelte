  ...
  function addState() {
    ...
    states = [
      ...states,
      {
        key: nextKey,
        label: "New Stage",
        color: colors[states.length % colors.length],
      },
    ];
  }
...
<div class="space-y-5">
  <div>
    ...
    <p class="mt-0.5 text-xs text-slate-500">
      Set the stages your team uses to qualify leads.
    </p>
  </div>
  ...
  {:else if !pipeline}
    ...
    >
      No lead pipeline is configured.
    </p>
  {:else}
    <div class="space-y-2.5">
      {#each states as state, index (state.key || index)}
        <div
          class="flex items-center gap-3 rounded-xl border border-slate-200/90 bg-white p-2.5 sm:p-3 shadow-2xs transition hover:border-slate-300"
        >
          <!-- Styled Color Swatch Picker -->
          <div
            class="relative w-7 h-7 rounded-lg overflow-hidden border border-slate-200/80 shadow-2xs shrink-0 flex items-center justify-center bg-slate-50 cursor-pointer"
            title="Change color"
          >
            <span
              class="w-4 h-4 rounded-full border border-black/10 shadow-2xs"
