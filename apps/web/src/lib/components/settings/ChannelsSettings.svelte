	...
	<div class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/40 p-4" role="presentation" onclick={(event) => { if (event.currentTarget === event.target) closeDialog(); }}>
		<div class="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="connect-channel-title">
			...
			{:else if activeConnection.state === 'awaiting_scan'}
				...
				<div class="flex justify-end gap-2"><button onclick={() => { qrRefreshToken = Date.now(); void refresh(true); }} class="wf-button px-3 py-2 text-slate-600 hover:bg-slate-100">Refresh QR</button><button onclick={closeDialog} class="wf-button-primary px-3 py-2">Confirm QR scan</button></div>
			...
			{:else if activeConnection.state === 'awaiting_session'}
				<div class="my-5 space-y-3 text-xs leading-5 text-slate-600"><p>1. Open {activeConnection.platform === 'instagram' ? 'instagram.com' : 'messenger.com'} in a private browser window and sign in.<br />2. Open browser developer tools.<br />3. Copy an authenticated GraphQL request as POSIX cURL.</p><label class="block font-medium text-slate-700">Copied cURL request<textarea bind:value={secret} autocomplete="off" spellcheck="false" class="wf-input mt-1.5 min-h-32 font-mono text-[11px]" placeholder="Paste the copied cURL request"></textarea></label></div><div class="flex justify-end gap-2"><button onclick={closeDialog} class="wf-button px-3 py-2 text-slate-600 hover:bg-slate-100">Cancel</button><button onclick={submitSession} disabled={busy || !secret.trim()} class="wf-button-primary px-3 py-2">{busy ? 'Handing off…' : 'Connect'}</button></div>
			{:else if activeConnection.state === 'connected'}<div class="my-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-xs leading-5 text-emerald-800">{platformName(activeConnection.platform)} is connected. New messages will sync automatically.</div><div class="flex justify-end"><button onclick={closeDialog} class="wf-button-primary px-3 py-2">Done</button></div>
