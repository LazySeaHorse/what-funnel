...
<div class="mb-3 mt-4 flex items-center justify-between w-full">
	<div class="flex items-center gap-2">
		<h3 class="text-xs font-semibold uppercase tracking-wider text-slate-700">Extracted Concepts</h3>
		<span class="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-600 border border-slate-200/60">{concepts.length}</span>
	</div>
	...
	{#each concepts as concept (concept.id)}
		<div class="p-4 bg-white border rounded-2xl space-y-3 transition-all {concept.approved ? 'border-slate-200/80 shadow-2xs hover:border-slate-300' : 'border-slate-200/50 bg-slate-50/50 opacity-60'}">
			<div class="flex items-center gap-2">
				...
				<input bind:value={concept.type} disabled={!concept.approved} aria-label="Concept type" placeholder="Type" class="w-24 border rounded-xl px-2.5 py-1.5 text-[11px] font-semibold outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 disabled:opacity-50 transition text-center capitalize truncate {concept.approved ? typeColor(concept.type) : 'bg-slate-100 text-slate-400 border-slate-200'}" />
			</div>
...
<div class="mb-3 mt-6 flex items-center justify-between w-full">
	<div class="flex items-center gap-2">
		<h3 class="text-xs font-semibold uppercase tracking-wider text-slate-700">Direct Answer Patterns</h3>
